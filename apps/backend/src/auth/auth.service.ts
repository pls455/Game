...
type TokenUser = { id: string; email: string; full_name: string };
...
export class AuthService {
  ...
  async login(email: string, password: string) {
    const result = await this.db.query(
      `SELECT id,email,full_name,password_hash FROM users WHERE email=$1 AND is_active=true`,
  ...
  async refresh(refreshToken: string) {
    ...
    const result = await this.db.query(
      `SELECT u.id,u.email,u.full_name FROM refresh_tokens r JOIN users u ON u.id=r.user_id WHERE r.token_hash=$1 AND r.revoked_at IS NULL AND r.expires_at>now() AND u.is_active=true`,
      [hash],
    );
